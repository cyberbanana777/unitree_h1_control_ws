...
] = "hide"  # Disable pygame welcome message
import time
...
class Axis:
    ...
    def __call__(self, data) -> None:
        ...
        self.on_pressed = (
            self.pressed and math.fabs(self.data) < self.threshold
        )
        self.on_released = (
            not self.pressed and math.fabs(self.data) > self.threshold
        )
        self.data = new_data
...
class Joystick:
    ...
    def extract(self, wireless_remote):
        ...
        if any(
            ...
            time.perf_counter() - self.last_active_time > self.inactive_timeout
        ):
