    ...
    try:
        with open(filename, "rb") as f:
            ...
            if byte_rate != expected_byte_rate:
                print(
                    f"[ERROR] byte_rate mismatch: got {byte_rate}, expected {expected_byte_rate}"
                )
                return [], -1, -1, False
            ...
            if block_align != expected_align:
                print(
                    f"[ERROR] block_align mismatch: got {block_align}, expected {expected_align}"
                )
                return [], -1, -1, False

            if bits_per_sample != 16:
                print(
                    f"[ERROR] Only 16-bit samples supported, got {bits_per_sample}"
                )
                return [], -1, -1, False
...
def write_wave(filename, sample_rate, samples, num_channels=1):
    try:
        ...
        with open(filename, "wb") as f:
            ...
            f.write(
                struct.pack("<I", sample_rate * num_channels * 2)
            )  # byte_rate
...
def play_pcm_stream(
    ...
    stream_id = str(
        int(time.time() * 1000)
    )  # Unique stream ID based on current timestamp
    offset = 0
    chunk_index = 0
    ...
    while offset < total_size:
        ...
        if verbose:
            ...
            print(
                f"[CHUNK {chunk_index}] offset = {offset}, size = {current_chunk_size} bytes"
            )
        ...
        if ret_code != 0:
            print(
                f"[ERROR] Failed to send chunk {chunk_index}, return code: {ret_code}"
            )
            break
        else:
